...
const char *Algorithm::kRX_QMR          = "rx/qmr";
#endif
...
#   ifdef XMRIG_ALGO_RANDOMX
    ...
    ALGO_ALIAS_AUTO(RX_QMR),        ALGO_ALIAS(RX_QMR,          "randomx/qmr"),
                                    ALGO_ALIAS(RX_QMR,          "rx/qmr"),
#   endif
...
std::vector<xmrig::Algorithm> xmrig::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
    static const std::vector<Id> order = {
        ...
        CN_UPX2,
        RX_0, RX_WOW, RX_ARQ, RX_SFX, RX_KEVA, RX_QMR,
